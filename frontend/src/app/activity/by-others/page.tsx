...
export default function OthersActivitiesPage() {
  ...
  const [uniqueUsers, setUniqueUsers] = useState<Array<{id: number, username: string}>>([]);

  useEffect(() => {
    fetchActivities();
  }, [searchTerm, actionFilter, entityFilter, userFilter, currentPage]);
  ...
  const fetchActivities = async () => {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        limit: '20',
        excludeUserId: user.id.toString(), // Exclude current user's activities
      });
      ...
      const users = Array.from(
        new Set(response.data.data?.map((a: Activity) => a.user.id) || [])
      ).map(userId => {
        const activity = response.data.data?.find((a: Activity) => a.user.id === userId);
        return { id: userId, username: activity?.user.username || '' };
      });
      setUniqueUsers(users);
    } catch (error) {
      toast.error('Failed to load others\' activities');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-white">Others' Activities</h1>
          <p className="text-zinc-400 mt-1">
            Activity history from other users in the system
          </p>
        </div>
      </div>
      ...
      <Card className="bg-zinc-900/50 border-zinc-700/50">
        <CardHeader>
          <CardTitle className="text-white">Other Users' Activities</CardTitle>
        </CardHeader>
        <CardContent>
          {activities.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-zinc-500 text-sm mt-1">
                Other users' activities will appear here
              </p>
            </div>
          ) : (
            <>
