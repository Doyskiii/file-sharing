...
async function checkPasswordHash() {
  try {
    ...
    // Check the password hash for Superadmin user
    const result = await client.query('SELECT id, username, email, password FROM users WHERE email = $1;', ['Superadmin@example.com']);
