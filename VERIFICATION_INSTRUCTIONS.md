# Instruksi untuk Restart dan Verifikasi Setelah Perbaikan

## Perubahan yang Telah Dilakukan

1. **Menambahkan kolom `folderId` ke model Activity** di `app/models/activity.ts`
2. **Memperbarui fungsi `log` di ActivityService** untuk hanya menetapkan `folderId` jika itu benar-benar disediakan
3. **Meningkatkan limit multipart** di `config/bodyparser.ts` dari 20MB ke 50MB
4. **Menambahkan endpoint debug** `/debug-files` untuk membantu debugging upload
...
- Upload file akan berhasil tanpa error "Cannot define folderId on Activity model"
- File akan disimpan ke database dan sistem file
- Aktivitas upload akan dicatat di tabel activities